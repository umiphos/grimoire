...
require('PDF/lib/PDF/fpdf.php');
	...
	while($control!=0){
		...
		$resultados = mysql_query("
		SELECT FNACIMIENTO,NOMBRES,APATERNO,AMATERNO,SEXO,COLONIA,TCASA,personas.IDPERSONA,PUESTO,MUNICIPIO,
		FECHA,COMITE,CALLE,NUMERO,localizacion.CP,personas.FOTOGRAFIA
		FROM miembros
		INNER JOIN comites ON miembros.IDCOMITE = comites.IDCOMITE
		INNER JOIN personas ON miembros.IDPERSONA = personas.IDPERSONA
		INNER JOIN localizacion ON miembros.IDLOCALIZACION = localizacion.IDLOCALIZACION
		...
		while($row=mysql_fetch_array($resultados)){
			if ($salida!=0){
				...
				$pdf->Image("FOTOS/$foto",60,43,21,21);
				$pdf->AddFont('colibri_negro','','colibri.php');
				$pdf->AddFont('colibri_negro');
				$pdf->SetFont('colibri_negro','',8);
				$pdf->Ln(10);
				$pdf->text(70,28,'COMITÉ VOLUNTARIO DE PROMOCIÓN');	
				//$pdf->Ln(3);
				$pdf->text(70,30,'Y DEFENSA DE LOS DERECHOS HUMANOS');	
				//$pdf->Ln(3);
				$pdf->text(70,32,'DE LA COLONIA'.$colonia);	
				$pdf->SetFont('colibri_negro','',16);
				$pdf->text(85,60,''.$nombreS.' '.$nombreP.' '.$nombreM);	
				$pdf->text(110,65,''.$cargo);	
				//$pdf->Ln(40);
				}
				if ($wasd==1){
				$pdf->Image('PDF/imagenes/creden.jpg',50,150,100,120);
				$pdf->Image("FOTOS/$foto",60,173,21,21);
				//$pdf->Image('PDF/imagenes/creden.jpg',50,35,100,100);
				$pdf->Ln(30);
				...
				$pdf->SetFont('colibri_negro','',8);
				$pdf->Ln(10);
				$pdf->text(70,158,'COMITÉ VOLUNTARIO DE PROMOCIÓN');	
				//$pdf->Ln(3);
				$pdf->text(70,160,'Y DEFENSA DE LOS DERECHOS HUMANOS');	
				//$pdf->Ln(3);
				$pdf->text(70,162,'DE LA COLONIA'.$colonia);	
				$pdf->SetFont('colibri_negro','',16);
				$pdf->text(85,180,''.$nombreS.' '.$nombreP.' '.$nombreM);	
				$pdf->text(110,185,''.$cargo);		
				}
				/*$pdf->Ln(25);
				$pdf->Cell(0,0,''.$nombreS.' '.$nombreP.' '.$nombreM,0,0,'L');
				$pdf->Ln(5);
